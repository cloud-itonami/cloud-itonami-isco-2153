(ns telecomeng.actor
  "TelecommunicationsEngineersActor — the ISCO-08 2153 community telecomeng actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one telecomeng operation request
  ...
  The unconditional invariant: the TelecommunicationsEngineersAdvisor can never
  directly commit a record the TelecommunicationsEngineersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [telecomeng.advisor :as advisor]
            [telecomeng.governor :as governor]
            [telecomeng.store :as store]))

(defn build-graph
  "Build a compiled TelecommunicationsEngineersActor graph. `store` implements
  `telecomeng.store/Store`. `advisor` implements
  `telecomeng.advisor/Advisor` (defaults to `mock-advisor`).
